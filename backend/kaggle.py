...
command = f'kaggle datasets download -d {dataset_name} -p dataset/images'

# run the kaggle command to download the file
subprocess.run(command, shell=True)

# unzip the downloaded file
zip_file_path = os.path.join('dataset', 'images', f'{dataset_name.split("/")[1]}.zip')
output_dir = os.path.join('dataset', 'images')
